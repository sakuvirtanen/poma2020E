...
Bond_price <- function(Begin_Date, Date, Maturity, Yield, Freq, Coupon) {
  ...
  PV = PV + (Coupon/F_ + 1)/(1+Yield/F_)^(max((interval(Date, Maturity) %/% months(1))/(12/F_),0))
  ...
  while (interval(Begin_Date, Wip_date) > 0 && Step_limit < 100) {
    ...
    PV = PV + (Coupon/F_)/(1+Yield/F_)^(max((interval(Date, Wip_date) %/% months(1))/(12/F_),0))
